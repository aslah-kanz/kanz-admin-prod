...
import { useAddJobField } from '@/api/job.api';
...
type TSheetAddJobFieldProps = {
  open: boolean;
  setOpen: Dispatch<SetStateAction<boolean>>;
};

export default function SheetAddJobField({
  open,
  setOpen,
}: TSheetAddJobFieldProps) {
  ...
  // add job field
  const { mutate: addJobField, isLoading: loadingAddJobField } = useAddJobField(
    {
      onSuccess: (resp) => {
        if (resp.code === HTTP_STATUS.SUCCESS) {
          toast.success('success to add job field');
          queryClient.invalidateQueries({ queryKey: ['job-fields'] });
          setOpen((prev) => !prev);
          form.reset();
        } else {
          toast.error('failed to add job field', {
