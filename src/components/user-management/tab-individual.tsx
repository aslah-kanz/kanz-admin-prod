...
import DetailIndividualSheet from '../individual/detail-individual-sheet';

export default function TabIndividual() {
  // const [showDetailIndividual, setShowDetailIndividual] =
  ...
  const { data: individualsList, isLoading: loadingData } = useGetPrincipals({
    type: searchParams.get('type') || 'individual',
  ...
  const { mutate: deleteIndividual } = useDeletePrincipals();
  ...
  }, [deleteIndividual, queryClient, router, selectedId]);
  ...
  return (
    <>
      ...
      {loadingData ? (
      ...
      ) : individualsList && individualsList?.content?.length ? (
        <div className=" mt-4 overflow-hidden rounded-lg border">
          <Table className="whitespace-nowrap [&_tr:nth-child(even)]:bg-neutral-100">
            ...
            <TableBody>
              {individualsList?.content?.map((items) => (
      ...
      <BasePagination
        totalCount={individualsList?.totalCount as number}
        customSizeOptions={[10, 20, 40]}
        totalDataTitle="user"
      />

      {/* begin: drawer detail individual customer */}
      <DetailIndividualSheet />
      {/* end: drawer detail individual customer */}
