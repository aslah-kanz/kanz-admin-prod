...
export default function PrivacyPolicyPage() {
  ...
  // fetch
  const { data, isLoading: loadingGet } = useGetWebPageBySlug('privacy-policy');
  ...
  const { mutate: addMutation, isLoading: loadingAdd } = useAddWebPage({
    onSuccess: (resp) => {
      if (resp.code === HTTP_STATUS.SUCCESS) {
        toast.success('success to add privacy policy');
        queryClient.invalidateQueries({
          queryKey: ['web-pages', { slug: 'privacy-policy' }],
        });
      } else {
  ...
  const { mutate: editMutation, isLoading: loadingEdit } = useEditWebPage({
    onSuccess: (resp) => {
      if (resp.code === HTTP_STATUS.SUCCESS) {
        toast.success('Success to edit privacy policy');
        queryClient.invalidateQueries({
          queryKey: ['web-pages', { slug: 'privacy-policy' }],
        });
      } else {
        toast.error('Failed to edit privacy policy', {
          description: resp.message,
        });
      }
    },
