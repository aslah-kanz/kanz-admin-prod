...
import ExchangeDetailLoading from '@/components/exchange-management/exchange-detail-loading';
...
export default function ExchangeDetailPage({ params }: TOrderDetailPageParams) {
  const { data, isLoading } = useGetExchangeById(params.exchange_id);
  ...
  const actualSubmitReject: SubmitHandler<TSchema> = useCallback(
    (values) => {
      mutateReject(
        { id: params.exchange_id, payload: values },
        {
          onSuccess: (resp) => {
            if (resp.code === HTTP_STATUS.SUCCESS) {
              toast.success('Reject exchange success');
              queryClient.invalidateQueries({ queryKey: ['exchanges'] });
              setIsOpenReject(false);
            } else {
              toast.error('Reject exchange failed', {
    ...
    [mutateReject, params.exchange_id, queryClient],
  );
  ...
  const actualSubmitApprove: SubmitHandler<TSchema> = useCallback(
    (values) => {
      mutateApprove(
        { id: params.exchange_id, payload: values },
        {
          onSuccess: (resp) => {
            if (resp.code === HTTP_STATUS.SUCCESS) {
              toast.success('Approve exchange success');
              queryClient.invalidateQueries({ queryKey: ['exchanges'] });
              setIsOpenApprove(false);
            } else {
              toast.error('Approve exchange failed', {
                description: resp.message,
              });
            }
          },
    ...
    [mutateApprove, params.exchange_id, queryClient],
  );
              ...
              <div className="flex flex-col items-center justify-center gap-4">
                ...
                <div className=" space-y-1 text-center">
                  ...
                  <p className=" text-center text-sm text-neutral-500">
                    Are you sure you want to proceed with rejecting the exchange
            ...
            >
              <div className="flex flex-col items-center justify-center gap-4">
                ...
                <div className=" space-y-1 text-center">
                  ...
                  <p className=" text-center text-sm text-neutral-500">
                    Are you sure you want to proceed with approving the exchange
                    request? This action is irreversible
                  </p>
                </div>
